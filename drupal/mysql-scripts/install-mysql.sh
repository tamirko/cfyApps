...
function killMySqlProcess {
	ctx logger info "${currHostName}:$0 sudo service mysql stop..."
	sudo service mysql stop
	ps -ef | grep -iE "mysqld" | grep -ivE "grep|cfy|cloudify"
	if [ $? -eq 0 ] ; then 
		ctx logger info "${currHostName}:$0 xargs sudo kill -9 mysql..."
...
if sudo grep -q -E '[^!]requiretty' /etc/sudoers; then
	ctx logger info "${currHostName}:$0 Setting requiretty..."
    echo "Defaults:`whoami` !requiretty" | sudo tee /etc/sudoers.d/`whoami` >/dev/null
    ctx logger info "${currHostName}:$0 Chmodding /etc/sudoers.d..."
...
ctx logger info "${currHostName}:$0 Invoking apt-get -y -q update ..."
sudo apt-get -y -q update || error_exit $? "Failed on: sudo apt-get -y update"

ctx logger info "${currHostName}:$0 #1 Killing old mysql process if exists..."
killMySqlProcess

# Removing previous mysql installation if exists

ctx logger info "${currHostName}:$0 apt-get -y -q purge mysql..."
...
ctx logger info "${currHostName}:$0 Removing old stuff if exists..."
...
ctx logger info "${currHostName}:$0 Using apt-get. Updating apt-get on one of the following : Ubuntu, Debian, Mint" 
...
ctx logger info "${currHostName}:$0 Killing old mysql process if exists b4 ending the installation..."
killMySqlProcess
...
if [ $sectionNamesLen -gt 0 ] ; then
	...
	if [ -f "${myCnfPath}" ] ; then
		...
		ctx logger info "${currHostName}:$0 IFS is ${IFS}"
		ctx logger info "${currHostName}:$0 ${sectionNamesArr[@]}"
		ctx logger info "${currHostName}:$0 ${variableNamesArr[@]}"
		ctx logger info "${currHostName}:$0 ${newValuesArr[@]}"
		...
		do
			...
			ctx logger info "${currHostName}:$0 Commenting $currVariable in $myCnfPath ... "
			sudo sed -i -e "s/^$currVariable/#$currVariable/g" $myCnfPath
			
			jointStr="${currSection}\n${currNewLine}"
			ctx logger info "${currHostName}:$0 Setting ${currNewLine} in the ${currSection} section of $myCnfPath ... "
