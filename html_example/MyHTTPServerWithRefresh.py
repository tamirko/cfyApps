...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def show_deplyoments(self, path):
        ...
        deployment_str += "<row>"
        actions = ""
        action1_display = "Add a vCPE"
        action2_display = "Update a vCPE"
        action3_display = "Create a FireWall"
        action4_display = "Create a Router"
        ...
        actions += "<td><button type=\"button\" onclick=\"alert('{0}!')\">{0}</button></td>".format(action4_display)
...
#       deployment_str += "<hr>"
        ...
        for deployment in cloudify_client.deployments.list(blueprint_id=blueprint_id):
            ...
            deployment_str += "<li><{1} class=\"{7}\">{0} ({5}) - {2} {3} {6} {4} {8}</{1}></li>".format(deployment_id, "h4", curr_deploy, curr_update, curr_undeploy, curr_status, curr_run_wf, "deployment_name", curr_delete)
...
#            deployment_str += "<{2}>--{0} {1}--</{2}>".format(deployment_id, 'outputs', "h5")
            deployment_str += "{0}".format(created_at_msg)
