...
dbName=$(ctx target node properties dbName)
...
dbUser=$(ctx target node properties dbUserName)
ctx logger info "${currHostName}:${currFilename} :dbUser is ${dbUser}"

dbPassW=$(ctx target node properties dbUserPassword)
...
ctx logger info "${currHostName}:${currFilename} schemaurl of the target node is ${currSchema} ... "
